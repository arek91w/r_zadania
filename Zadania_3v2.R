...
KNNtrain <- function( X, y_tar, k, XminNew, XmaxNew){
  ...
  # nie wiedziałem jak sprawdzić jednocześnie czy matrix czy data frame
  # sprawdzenie poprawnosci argumentow funkcji
  stopifnot(nrow(X) == nrow(y_tar),
            !is.na(X),
            !is.na(y_tar),
            k > 0,
            is.data.frame(X))
  
  MinMax <- function( x, XminNew, XmaxNew){ #Nie wiem jak wrzucić parametry funkcji nadrzędnej do funkcji wewnętrznej
    (x - min(x)) / (max(x)-min(x)) * (XminNew - XmaxNew) 
  }

  #if (){}               #nie wiem jak dodać ten warunek Jest to możliwe dla zmiennych na skali ilorazowej.

  X_new <- as.data.frame(lapply(X, MinMax))
  X_new
...
#    Euklidesa, Hamminga lub Gowera. Jezeli jedna factor to musi byc Gover. 
...
#    - tylko zmienne na skali porządkowej.factor uporzadkowany , sprawdzic przez atrybut order
#    - tylko zmienne na skali nominalnej. factor (albo tekst, albo liczby)
...
KNNpred <- function(KNNmodel, X ) {
  ...
  # sprawdzenie poprawnosci argumentow funkcji
  stopifnot(nrow(KNNmodel$X) == nrow(X),
            names(KNNmodel$X) %in% names(X)
            !is.na(X))
  
  # e) y_tar: regresja -> numeric, klasyfikacja -> factor.
